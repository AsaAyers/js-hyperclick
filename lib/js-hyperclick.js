...
import type { TextEditor } from './atom-types'
...
function makeProvider(subscriptions) {
    ...
    const followSuggestionPath = (fromFile, suggestion) => {
        ...
        // This is awkward, I can't just do `= resolved.url` because that prop
        // doesn't exist if it resolves to a `.filename`. But I have to extract
        // url into its own variable because flow doesn't trust that
        // `atom.packages.isPackageLoaded()` won't change resolved.url. See
        // Refinement Invalidations https://flow.org/en/docs/lang/refinements/
        const url: ?string = (typeof resolved.url === 'string' ? resolved.url : null)
        if (url != null) {
        ...
        // See `url` above
        const filename = (typeof resolved.filename === 'string' ? resolved.filename : null)
        if (filename == null) {
