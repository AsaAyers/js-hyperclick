"use babel"
// @flow
import type { Info, Range, Suggestion, SuggestionFromImport } from '../types'
...
export default function buildSuggestion(info: Info, text: string, { start, end }: Range, options: SuggestionOptions = {}): ?Suggestion {
    ...
    for (let i = 0; i < paths.length; i++) {
        ...
        if (path.range.start <= start && path.range.end >= end) {
            ...
            if (path.imported !== 'default') {
                const tmp: SuggestionFromImport = {
                    type: 'from-import',
