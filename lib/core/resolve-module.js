...
import type { Suggestion, Resolved } from '../types'
...
export default function resolveModule(filePath: string, suggestion: Suggestion, options: ResolveOptions = {}): Resolved {
    const { extensions = defaultExtensions } = options
    let { moduleName } = suggestion
    if (moduleName == null) throw new Error('Missing moduleName')
